...
### Phase 2 fix round 1: multi-outDir mapping; index summary

From the supabase run (`work/sentei.db`, the supabase-js and postgres-meta
clones). Entry-point changes are in `packages/core/src/manifests.ts`; the
summary is in `packages/cli/src/stages/index.ts` and `main.ts`.

- **Several tsconfig outDirs.** auth-js, functions-js, realtime-js and
  tracing (in supabase-js) declare `main: dist/main/index.js`,
  `module: dist/module/index.js` and `types: dist/module/index.d.ts`, built by
  `tsconfig.json` (outDir `dist/main`) and `tsconfig.module.json` (extends
  `./tsconfig`, outDir `dist/module`), both rootDir `src`. The dist→src
  convention mapped `dist/main/index.js` to `src/main/index.ts`, which does not
  exist. So no entry point resolved, each package got three `opaque_consumer`
  flags ("discover: unresolved entry point dist/main/index.js" …; six for
  tracing), and every finding behind them was blocked. This was the top
  blocker of the run, with 134 findings blocked. Entry resolution now reads every
  package-root `tsconfig*.json`, follows relative `extends` (a string or an
  array, `./x` or `./x.json`, paths relative to the defining config, JSONC,
  parse failures ignored) and maps an entry under an outDir to the same path
  under its rootDir. The rootDir default is the single non-glob `include`, else
  `src`, else the package dir. Source extensions include `.mts`/`.cts` and
  allowJs `.js`. The old convention is the next fallback, and after it comes a
  one-segment strip (`dist/<seg>/x` → `src/x`). The strip applies only when
  nothing exists at `src/<seg>` (otherwise `<seg>` is a subpath such as
  `dist/react/`) and never for a `*` pattern (`dist/gone/*.js` → `src/*.ts`
  would match every source). On the clone, all four packages now resolve to
  `src/index.ts` with no unresolved entries. Fixture: `lib-dual` (`@acme/dual`
  plus the consumer `@acme/dual-app`). With the old mapping `dualUnused` is
  blocked by `opaque_consumer`; with the new one it is a `deletion_candidate`.
  Only new snapshots were added; the adapter version is unchanged.
- **Runtime entries from scripts and Dockerfiles.** postgres-meta starts
  `node dist/server/server.js` (`scripts.start` and the Dockerfile `CMD`) and
  declares only `src/lib` as its entry. The server was unreachable, which
  produced a false dead_island DEPRECATE on `PostgresMeta`, 7 false org_dead
  helpers and 71 false PRIV-DEAD rows. Two sources now add runtime entries:
  the file after a runner (node, tsx, ts-node, bun, deno, nodemon, vite-node,
  …) in any package.json script, and the `CMD`/`ENTRYPOINT` of a package-root
  Dockerfile (exec or shell form, ENTRYPOINT+CMD combined, WORKDIR-absolute
  paths made relative, `HEALTHCHECK … CMD` ignored). Both go through the same
  build-output→source mapping. Flags and their values are skipped, and
  `-e`/`-p` inline code yields nothing. On the clone the entries are now
  `src/lib/index.ts` and `src/server/server.ts`. Fixture:
  `lib-dual/packages/dual-server`.
- **Next.js files loaded by name.** With a `next` dependency, `next.config.*`
  (at the root) is an entry. So are `middleware`, `proxy`, `instrumentation`,
  `instrumentation-client` and `mdx-components`, in the dir Next reads them
  from: `src/` when the router is `src/app` or `src/pages` and there is no root
  `app/`/`pages/`, else the root. A root `middleware.ts` next to a `src/app`
  router is ignored by Next, and sentei does the same. Fixture:
  `lib-dual/packages/dual-web`.
...
- **Index summary; `--strict`.** Failed packages already fail closed
  ...
  failed / partial counts per indexer, plus one line per failed package: its
  id, the first diagnostic line that names a cause (a `TSnnnn`, an `Error:`,
  heap exhaustion; stack frames, echoed source and pipe-joined stderr tails are
  skipped) and its log path. A cached failure counts as failed. `--strict`
  (index, run) exits 2 when anything failed. `run --strict` finishes every
  stage first, so the report exists. Exit code 2 is shared with usage errors (the
