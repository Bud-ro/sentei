...
| `lib-dual` | `@acme/dual`, `@acme/dual-app`, `@acme/dual-legacy`, `@acme/dual-server`, `@acme/dual-web`, `@acme/dual-script`, `_unnamed/unnamed-demo` | private | two-package repo: `dual` declares only build output under two tsconfig outDirs (`dist/main`, `dist/module`, both rootDir `src`: the supabase auth-js layout), `dual-app` consumes it. Entry points map to `src/index.ts`, so `dualUnused` is a `deletion_candidate` rather than blocked by `opaque_consumer`. `@acme/dual-server` (no `main`) is started by `npm start` = `node dist/server.js` and a Dockerfile `CMD ["node", "/srv/dist/worker.js"]`: both map to `src/` and are runtime entries (the supabase postgres-meta layout). `@acme/dual-web` is a Next.js `src/app` project: `src/middleware.ts`, `src/instrumentation.ts` and `next.config.ts` are entries loaded by name; the root `middleware.ts` is not (Next ignores it beside `src/app`), so its export is `private_dead`. `@acme/dual-script` (`exports` → `src/index.ts`, tsconfig `include: ["src"]`) has runtime entries outside its program: `npm start` = `node scripts/serve.mjs`, Next.js `next.config.mjs` and a CommonJS `bin/cli.cjs`; they are indexed through the runtime tsconfig and never make it opaque, so `scriptLibUnused` is a `deletion_candidate`, the script-only `serveBanner` is alive and the require binding `banner` is alive. `unnamed-demo/` has a package.json without `"name"`: the consumer-only package `_unnamed/unnamed-demo`, whose use keeps `@acme/dual`'s `dualUsedByUnnamed` alive |
