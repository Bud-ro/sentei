...
| `bin/` re-exports `main` (over_react_codemod's executables) | `acme_tools/bin/acme_tools.dart` = `export 'package:acme_tools/src/cli.dart';` | `main` in `lib/src/cli.dart` and `_banner` alive (sidecar `entrySymbols` from the bin library's export namespace; they were private_dead) |
...
| Conditional export (supabase_common `default_http_client_*.dart`) | `acme_core/lib/platform.dart`: `export 'src/platform_stub.dart' if (dart.library.io) 'src/platform_io.dart' if (dart.library.js_interop) 'src/platform_web.dart';`, used by `acme_tools` | every `platformName` and the io variant's `_ioDetail` alive: the io/web twins take the stub's export surface (`directive: 'export'`); before, they were private_dead already_unreachable |
