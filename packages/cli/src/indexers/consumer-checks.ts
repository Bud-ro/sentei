 ...
 * "do not modify by hand" (Dart source_gen / build_runner's `// GENERATED CODE
 * - DO NOT MODIFY BY HAND`), "auto generated" (ffigen's `// AUTO GENERATED
 * FILE, DO NOT EDIT.`, jnigen's `// AUTO GENERATED BY JNIGEN ...`). The Dart
 * adapter (scip-dart.ts) runs the same sniff over its documents. Round 7
 * (dart-lang): "do not modify" alone (protoc-gen-dart's `//  Generated code. Do
 * not modify.`, "by hand" or not) and "generated from … IDL" (package:web's `//
 * Generated from Web IDL definitions.`).
 */
const GENERATED_HEADERS: readonly RegExp[] = [
  /@generated|automatically generated|auto[- ]generated|do not edit|do not modify/i,
  /generated by /i,
  /this file was generated/i,
  /code generated/i,
  /generated from\b.*\bIDL\b/i,
];
 ...
 * it lies in a vendored dir (core VENDORED_GLOBS: `third_party/`, `vendor/`,
 * `vendored/`) BELOW the root of its package `pkgPath` (repo-relative, '.' for
 * the repo root; core `inVendoredDir`, the rule of the `vendored_files` view, so
 * the sidecar's generatedFiles and core agree; a package whose own root sits
 * under `vendor/` is org code, and without `pkgPath` the vendored rule is not
 * applied),
 ...
 * start (read when absent); `headerLines` how many of its lines are read for
 * comments (the Dart adapter passes its whole leading comment header).
 */
export function isGeneratedFile(abs: string, repoRel: string, pkgPath?: string, head?: string, headerLines = HEADER_LINES): boolean {
  if (GENERATED_GLOBS.some((g) => matchGlob(g, repoRel))) return true;
  if (pkgPath !== undefined && inVendoredDir(repoRel, pkgPath)) return true;
  ...
  if (headerComments(text, headerLines).some((c) => GENERATED_HEADERS.some((re) => re.test(c)))) return true;
...
/** Comment texts within the first `maxLines` lines (`//`, `#`, `/* *\/`, `<!-- -->`, `{/* *\/}` in MDX). */
function headerComments(text: string, maxLines = HEADER_LINES): string[] {
  const out: string[] = [];
