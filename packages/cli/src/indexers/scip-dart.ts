...
export const scipDart: Indexer = {
  ...
  async run(input, outDir) {
    ...
    // the file's leading comment block counts (see [dartFileHeader]).
    const generatedFiles = documents
      .map((rel) => [path.join(dir, ...rel.split('/')), path.posix.join(pkg.path === '.' ? '' : pkg.path, rel)] as const)
      .filter(([abs, repoRel]) => isGeneratedFile(abs, repoRel, pkg.path, dartFileHeader(abs)))
      .map(([, repoRel]) => repoRel)
      .sort();
...
/**
 * The leading comment block of a Dart file (comment and blank lines before the
 * first directive or declaration), for the generated-header sniff; '' when
 * unreadable. The TypeScript sniff reads every comment of the first 20 lines,
 * but in Dart those include the `///` docs of the first declarations: "The
 * command line argument parser generated by the `build_cli_annotations`
 * package." (dart-lang/samples) or "will be automatically generated"
 * (swiftgen's config.dart) marked hand-written files generated. Generator
 * headers (source_gen, ffigen, jnigen, protoc) come before any code.
 */
