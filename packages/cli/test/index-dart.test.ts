...
import { isGeneratedFile } from '../src/indexers/consumer-checks.ts';
...
describe('pubspec_overrides.yaml writer', () => {
  ...
  it('the generated-header sniff reads only the leading comment block of a Dart file', () => {
    const f = path.join(root, 'header.dart');
    const gen = (body: string): boolean => {
      writeFileSync(f, body);
      return isGeneratedFile(f, 'lib/header.dart', '.', dartFileHeader(f));
    };
    ...
    // A block comment is not a Dart generator header either (and ends the header).
