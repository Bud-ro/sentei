...
import { checkedInFiles, generateOrgSnapshots, hasDart, ORGS, SNAPSHOTS, UPDATE_COMMAND } from '../../../scripts/update-snapshots.ts';
...
for (const org of ORGS) {
  ...
  describe.skipIf(org.dart && !HAS_DART)(`indexer snapshots of fixtures/${org.name}`, () => {
    it('match fixtures/snapshots byte for byte', async () => {
      const got = await generateOrgSnapshots(org);
