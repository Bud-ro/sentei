...
describe('discoverLocal on fixtures/org-small', () => {
  it('builds the model and writes it to the DB', () => {
    ...
    expect(all('SELECT package_id, repo, path, manager, name, version, visibility, is_library, entry_points FROM packages ORDER BY package_id')).toEqual([
      ...
      { package_id: 'npm:@acme/worker', repo: 'acme/app-worker', path: '.', manager: 'npm', name: '@acme/worker', version: '1.0.0', visibility: 'private', is_library: 0, entry_points: '["functions/api/hello.ts","src/worker.ts"]' },
    ...
    expect(all('SELECT * FROM package_deps ORDER BY consumer_package_id, dep_name')).toEqual([
      ...
      { consumer_package_id: 'npm:@acme/worker', dep_name: '@acme/widgets', dep_manager: 'npm', dep_constraint: '^1.0.0', resolved_package_id: 'npm:@acme/widgets', dev: 0 },
    ]);
